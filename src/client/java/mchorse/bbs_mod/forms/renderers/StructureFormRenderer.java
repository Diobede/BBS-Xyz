...
public class StructureFormRenderer extends FormRenderer<StructureForm>
{
    ...
    private final List<BlockEntry> blockEntities = new ArrayList<>();
    private final List<BlockEntry> animatedBlocks = new ArrayList<>();
    private final List<BlockEntry> tintedBlocks = new ArrayList<>();
    private mchorse.bbs_mod.forms.utils.VirtualBlockRenderView cachedView = null;
    ...
    public void renderInUI(UIContext context, int x1, int y1, int x2, int y2)
    {
        ...
        // Use VAO even if lights are enabled (performance priority)
        // We will emulate lighting by passing global light to the shader
        
        if (!optimize)
        {
            // Modo BufferBuilder: mejor iluminación (por bloque), peor rendimiento
        ...
        else
        {
            ...
            if (this.structureVao != null)
            {
                ...
                // Calculate effective light for the whole structure
                int light = LightmapTextureManager.MAX_BLOCK_LIGHT_COORDINATE;
                if (lightsEnabled)
                {
                    // If emitting light, use max light (or intensity mapped to lightmap)
                    // For UI/Preview, we usually want max light anyway, but let's be consistent.
                    // In UI, light is usually MAX.
                }
                
                ModelVAORenderer.render(shader, this.structureVao, matrices, tint.r, tint.g, tint.b, tint.a, light, OverlayTexture.DEFAULT_UV);
    ...
    protected void render3D(FormRenderingContext context)
    {
        ...
        else if (this.structureVao != null)
        {
            ...
            // Apply light emission setting
            mchorse.bbs_mod.forms.forms.utils.StructureLightSettings sl = this.form.structureLight.getRuntimeValue();
            boolean lightsEnabled = (sl != null) ? sl.enabled : this.form.emitLight.get();
            if (lightsEnabled && !context.isPicking())
            {
                 // If lights are enabled, force max light for the whole structure to emulate internal lighting
    ...
    private void renderStructure(MatrixStack stack, CustomVertexConsumerProvider consumers, int light, int overlay)
    {
        ...
        Vector3f pivot = calculateStructurePivot();
    ...
    private void renderStructureCulledWorld(FormRenderingContext context, MatrixStack stack, net.minecraft.client.render.VertexConsumerProvider consumers, int light, int overlay, boolean useEntityLayers)
    {
        ...
        if (cachedView == null)
        {
            java.util.ArrayList<mchorse.bbs_mod.forms.utils.VirtualBlockRenderView.Entry> entries = new java.util.ArrayList<>();
            for (BlockEntry be : blocks)
            {
                entries.add(new mchorse.bbs_mod.forms.utils.VirtualBlockRenderView.Entry(be.state, be.pos));
            }
            cachedView = new mchorse.bbs_mod.forms.utils.VirtualBlockRenderView(entries);
        }
        ...
        mchorse.bbs_mod.forms.utils.VirtualBlockRenderView view = cachedView
    ...
    private void renderBiomeTintedBlocksVanilla(FormRenderingContext context, MatrixStack stack, net.minecraft.client.render.VertexConsumerProvider consumers, int light, int overlay)
    {
        ...
        for (BlockEntry entry : tintedBlocks)
        {
            stack.push();
            stack.translate(entry.pos.getX() - pivotX, entry.pos.getY() - pivotY, entry.pos.getZ() - pivotZ);
    ...
    private void renderBlockEntitiesOnly(FormRenderingContext context, MatrixStack stack, net.minecraft.client.render.VertexConsumerProvider consumers, int light, int overlay)
    {
        ...
        for (BlockEntry entry : blockEntities)
        {
            Block block = entry.state.getBlock();
            ...
            if (be != null)
            {
                ...
                mchorse.bbs_mod.forms.utils.VirtualBlockRenderView beView = cachedView
                    .setBiomeOverride(this.form.biomeId.get())
    ...
    private void buildStructureVAO()
    {
        ...
        collector.setGenerateTangents(true);
    ...
    private Vector3f calculateStructurePivot()
    {
        float cx;
        float cy;
        float cz;
        ...
        PivotSettings pivotSettings = this.form.pivot.getRuntimeValue();
        boolean auto = (pivotSettings != null) ? pivotSettings.auto : this.form.autoPivot.get();

        if (!auto)
        {
            if (pivotSettings != null)
            {
                return new Vector3f(pivotSettings.pivot.x, pivotSettings.pivot.y, pivotSettings.pivot.z);
            }
            else
            {
                return new Vector3f(this.form.pivotX.get(), this.form.pivotY.get(), this.form.pivotZ.get());
            }
        }
        else
        {
            ...
            return new Vector3f(cx - parityXAuto, cy, cz - parityZAuto);
        }
    }
        ...
        if (root.contains("blocks", NbtElement.LIST_TYPE))
        {
            ...
            for (int i = 0; i < list.size(); i++)
            {
                ...
                if (stateIndex >= 0 && stateIndex < paletteStates.size())
                {
                    ...
                    BlockEntry entry = new BlockEntry(state, pos);
                    blocks.add(entry);

                    if (isAnimatedTexture(state))
                    {
                        animatedBlocks.add(entry);
                    }
                    if (isBiomeTinted(state))
                    {
                        tintedBlocks.add(entry);
                    }
